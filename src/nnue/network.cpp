...
void Network::verify(const std::function<void(std::string_view)>& f,
    ...
    if (evalFile.current != evalfilePath)
    {
        // F2 placeholder eval: no usable network — do not terminate. The
        // engine keeps running with the material fallback in Eval::evaluate
        // (nothing is downloaded; drop a matching .nnue next to the binary
        // or set EvalFile to restore NNUE evaluation).
        if (f)
            f("The network file " + evalfilePath.string()
              + " was not loaded: using material-eval fallback");
