...
Value Eval::evaluate(const Eval::NNUE::Network&     network,
    ...
    // F2 placeholder eval: if no network could be loaded (chassis build
    // without an embedded .nnue and no EvalFile), fall back to simple
    // material + tempo so the binary still runs (perft/bench/debug).
    if (!network.is_loaded())
    {
        static std::atomic_bool reported{false};
        if (!reported.exchange(true))
            sync_cout << "info string material-eval fallback" << sync_endl;

        Value mat = PawnValue * (pos.count<PAWN>(WHITE) - pos.count<PAWN>(BLACK))
        ...
        Value v = (pos.side_to_move() == WHITE ? mat : -mat) + 28;  // tempo
...
std::string Eval::trace(Position& pos, const Eval::NNUE::Network& network) {
    ...
    // F2 placeholder eval
    if (!network.is_loaded())
        return "Final evaluation: material-eval fallback (no NNUE network loaded)";
