...
class Worker {
   ...
   private:
    ...
    // ~256KB per search frame. Each ply gets ExtMove slots (main/qsearch
    // picker and the ProbCut picker) and the whole thread shares one
    // generation scratch, consumed within each next_move() call. Pages are
    // only committed when touched, so the large reservation is virtual.
